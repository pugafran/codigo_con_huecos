...
void procesa_argumentos(int argc,char *argv[])
{
    ...
    if (argc != 5) {
        ...
        printf("Uso: %s <num_puerto> <t|u> <tam_cola> <num_hilos_aten> <num_hilos_work>\n", argv[0]);
        exit(1);
    }
    ...
    if (num_puerto < 1024 || num_puerto > 65535) {
        printf("El número de puerto debe estar entre 1024 y 65535.\n");
        exit(1);
    }

    char tipo_socket = argv[2][0];
    if (tipo_socket != 't' && tipo_socket != 'u') {
        printf("El tipo de socket debe ser 't' o 'u'.\n");
        exit(1);
    }

    int tam_cola = atoi(argv[3]);
    if (tam_cola <= 0) {
        printf("El tamaño de la cola debe ser un número positivo.\n");
        exit(1);
    }

    int num_hilos_aten = atoi(argv[4]);
    if (num_hilos_aten <= 0) {
        printf("El número de hilos de atención debe ser un número positivo.\n");
        exit(1);
    }

    int num_hilos_work = atoi(argv[5]);
    if (num_hilos_work <= 0) {
        printf("El número de hilos trabajadores debe ser un número positivo.\n");
        exit(1);
    }
}
